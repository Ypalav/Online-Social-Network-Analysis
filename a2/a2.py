...
def token_pair_features(tokens, feats, k=3):
    ...
    for token in range(len(tokens)):
        temp_list = []
        if (len(tokens) >= (token + k)):
            count.update(list(combinations(tokens[token:token + k], 2)))
    str1 = "token_pair="

    for keys in sorted(count):
        feats[str1 + keys[0] + "__" + keys[1]] = count[keys]
...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    def featurize_local():
        ...
        for token in tokens_list:
            feats_local+= [featurize(token, feature_fns)]
        return feats_local
    if vocab is None:
        ...
        for tokens in feats:
            flag_ptr=0
            for t in tokens:
                new_vocab_list.append(t[0])
                if t not in new_dict or flag_ptr == 0:
                    new_dict[t[0]] += 1
                    flag_ptr = 1
        new_vocab_list = sorted(new_vocab_list)
        index_ptr = 0
        for token in new_vocab_list:
            if token not in vocab:
                vocab[token] = index_ptr
                index_ptr += 1
        k = 0
        new_feats = enumerate(feats)
        for keys, tokens in new_feats:
            for token in tokens:
                if new_dict[token[0]] >= min_freq:
                    data_values.append(token[1])
                    row_index.append(k)
                    col_index.append(vocab[token[0]])
            k += 1
        X = csr_matrix((data_values, (row_index, col_index)), shape=(len(tokens_list), len(vocab)), dtype=np.int64)
        return X, vocab
    else:
        feats = featurize_local()
        k = 0
...
def cross_validation_accuracy(clf, X, labels, k):
    ...
    for train, test in kfold:
        ...
        list_accuracy.append(accuracy_score(labels[test], predicted))
    avg = np.mean(list_accuracy)
    return avg
...
def eval_all_combinations(docs, labels, punct_vals,
    ...
    output_list=[]
    feature_list=[]
    for feature in range(1, len(feature_fns)+1):
        for f in combinations(feature_fns, feature):
            feature_list.append(f)

    for punct_val in punct_vals:
        tokens_list = [tokenize(d, punct_val) for d in docs]
        for new_feature in feature_list:
            for min_freq in min_freqs:
                output_dict = {}
                X, vocab = vectorize(tokens_list, new_feature, min_freq)
                average = cross_validation_accuracy(LogisticRegression(), X, labels, 5)
                output_dict['punct'] = punct_val
                output_dict['features'] = new_feature
                output_dict['min_freq'] = min_freq
                output_dict['accuracy'] = average
                output_list.append(output_dict)

    return sorted(output_list, key=lambda x: (-x['accuracy'], -x['min_freq']))
...
def mean_accuracy_per_setting(results):
    ...
    new_result = []
    for result in results:
        new_result.append(("features", result['features']))
        new_result.append(("min_freq", result['min_freq']))
        new_result.append(("punct", result['punct']))
    final = []
    for result in set(new_result):
        acc_res = []
        for r in results:
            if r[result[0]] == result[1]:
                acc_res.append(r['accuracy'])
        if 'features' in result[0]:
            f_name = [f.__name__ for f in result[1]]
            final.append((np.mean(acc_res), str(("features=" + ' '.join(f_name)))))
        else:
            final.append((np.mean(acc_res), (result[0] + "=" + str(result[1]))))
...
def top_coefs(clf, label, n, vocab):
    ...
    pos_coefs = []
    neg_coefs = []
    for val in zip(sorted(vocab.keys()), clf.coef_[0]):
        if val[1] < 0:
            neg_coefs.append((val[0], abs(val[1])))
        else:
            pos_coefs.append(val)
    if label == 0:
        return sorted(neg_coefs, key=lambda x: -x[1])[:n]
    else:
        return sorted(pos_coefs, key=lambda x: -x[1])[:n]
...
def print_top_misclassified(test_docs, test_labels, X_test, clf, n):
    ...
    for val in range(0, X_test.shape[0]):
        if predicted[val] != test_labels[val]:
            new_dict[val] = proba[val, predicted[val]]

    new_sorted_list = sorted(new_dict.items(), key=lambda x: x[1], reverse = True)
    for val in new_sorted_list[:n]:
        print("truth=", test_labels[val[0]], " predicted=", predicted[val[0]], " proba=", proba[val[0], predicted[val[0]]])
        print((test_docs[val[0]]))
