...
def bfs(graph, root, max_depth):
    ...
    """
    unvisited_nodes=[]
    node2distances=defaultdict(lambda: 0)
    node2num_paths=defaultdict(lambda: 0)
    node2parents=defaultdict(lambda: 0)
    ...
    while len(new_deque)>0:
        current_node = new_deque.popleft()
        for node in graph.neighbors(current_node):
            if node2distances[current_node]<max_depth:
                if node2distances[node] >= node2distances[current_node] + 1 and node != root:
                    node2num_paths[node] = node2num_paths[node] + 1
                    node2parents[node].append(current_node)
                if node not in unvisited_nodes and node != root:
                    node2distances[node] = node2distances[current_node] + 1
                    node2num_paths[node] = node2num_paths[node] + 1
                    node2parents[node]= [current_node]
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    for (node, distance) in node2distances:
        if(node!=root):
            if(node not in unvisited_node):
                temp_dict[node] +=1
                unvisited_node.append(node)
                for parent in node2parents[node]:
                    temp_dict[parent] += temp_dict[node] / node2num_paths[node]
                    result[tuple(sorted((node,parent)))] += temp_dict[node]/node2num_paths[node]
    return result
...
def approximate_betweenness(graph, max_depth):
    ...
    """
    edge_btw_dict=defaultdict(lambda:0.0)
    for node in graph.nodes():
        ...
        edge_btw_dict = dict(Counter(edge_btw_dict) + Counter(results))
    
    for result in edge_btw_dict:
        edge_btw_dict[result] = edge_btw_dict[result] / 2;
...
def partition_girvan_newman(graph, max_depth):
    ...
    components = [component for component in nx.connected_component_subgraphs(temp_graph)]
    list_of_edges = approximate_betweenness(temp_graph, max_depth)
    
    i = 0
    while len(components) == 1:
        temp_graph.remove_edge(*sorted(list_of_edges.items(), key=lambda x:(-x[1],x[0][0],x[0][1]))[i][0])
        components = [component for component in nx.connected_component_subgraphs(temp_graph)]
        i+=1
    return components
...
def get_subgraph(graph, min_degree):
    ...
    2
    """
    return graph.subgraph([node for node in graph.degree() if graph.degree(node)>=min_degree])
...
def norm_cut(S, T, graph):
    ...
    """
    norm_cut = (float(cut(S,T,graph))/volume(S,graph)) + (float(cut(S,T,graph))/volume(T,graph))
    return norm_cut
...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
        ...
        score_max_depth.append((depth, norm_cut(components[0].nodes(), components[1].nodes(), graph)))
    return score_max_depth
...
def make_training_graph(graph, test_node, n):
    ...
    remove_edges=sorted(graph.edges(test_node))[:n]
    train_graph.remove_edges_from(remove_edges)
    return train_graph
